/**
 * Doctor appointments page — view and manage upcoming bookings.
 */
...
function DoctorAppointmentsContent() {
  ...
  const fetchAppointments = async () => {
    try {
      const res = await api.get('/scheduling/doctor-appointments/');
  ...
  const handleCancel = async (appointmentId: number) => {
    ...
    setMessage(null);
    try {
      await api.post(`/scheduling/doctor-appointments/${appointmentId}/cancel/`);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {appointments.length === 0 ? (
        <EmptyState
          ...
          description="Set up your availability to start receiving bookings"
          action={{ label: 'Set Availability', href: '/doctor/availability' }}
        />
      ) : (
        <div className="space-y-8">
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl flex items-center justify-center text-white font-bold shadow">
                          {apt.patient_name
          ...
          {others.length > 0 && (
            <section>
              ...
              <div className="space-y-3">
                {others.map((apt) => (
                  <div key={apt.id} className="bg-white rounded-xl border border-gray-100 p-5 opacity-70">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h3 className="font-semibold text-gray-700">{apt.patient_name || 'Patient'}</h3>
